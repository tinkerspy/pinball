...
class IO {
  private:
    ...
    uint8_t selected = 0; 
    IO_Adafruit_NeoPixel *led_strip[NUM_LED_STRIPS];
  ...
  public:
    IO& begin( int pin_clock, int pin_pl, uint8_t *addr, uint8_t *inp, uint8_t *gate );
    IO& range( uint8_t row_max, uint8_t col_max );
    IO& select( int row, bool latch = false );
    IO& strip( uint8_t n, IO_Adafruit_NeoPixel &s );
